...
- **Alloha требует Referer у самого top-level запроса навигации** —
  ...
  сразу отдаёт HTTP 404 «Ошибка!». Нужно передавать `referer` как опцию
  `page.goto(url, { referer })` — это уходит напрямую в CDP
  `Page.navigate`. Подтверждено вручную через реальный браузер (открыть
  yani.tv → перейти по ссылке на alloha.yani.tv из него — 200; открыть
  ссылку напрямую без Referer — 404).
- **Alloha дополнительно гео-блокирует по IP сервера, а не только по
  Referer.** Даже с верным Referer страница отдаёт «Контент не найден или
  недоступен в вашем регионе» — воспроизведено вручную с IP европейского
  дата-центра (не Россия/СНГ). Vercel-функции (`iad1`, US) почти наверняка
  попадают под тот же блок. По этой же причине AniLibria в этом проекте
  намеренно стримится в БРАУЗЕРЕ пользователя (§6), а не с сервера —
  единственный способ сохранить гео реального зрителя без прокси-
  инфраструктуры.

  Решение — RU HTTP(S)-прокси для Puppeteer конкретно на шаге навигации
  Alloha (`ALLOHA_PROXY_SERVER`/`_USERNAME`/`_PASSWORD` в `.env.example`,
  подключается в `src/lib/extract/alloha.ts` через `launchBrowser(proxy)` +
  `authenticateProxy()` в `browser.ts`). Chromium не парсит логин/пароль из
  самого `--proxy-server` — авторизация идёт отдельным CDP-запросом
  (`page.authenticate()`) на событие "Proxy Authentication Required", до
  первой навигации. Без переменных окружения экстрактор работает как
  раньше, напрямую (на случай, если конкретный регион функции внезапно не
  под блоком). Взят дата-центровый RU-прокси (не резидентский) — дешевле,
  но есть шанс, что антипиратские системы дополнительно банят известные
  хостинг-ASN независимо от гео; статус не проверен в проде на момент
  записи. Проксируется только сама навигация Puppeteer (лёгкий трафик
  HTML/JS страницы-эмбеда) — Range-стриминг самих байт видео пользователю
  (`fetchAndProxy` в `proxy.ts`) через этот прокси НЕ идёт; если CDN с
  видео тоже окажется гео-блокирован отдельно от страницы-эмбеда, это
  придётся решать отдельно (и уже с учётом трафика в гигабайтах, а не
  килобайтах).
