...
- **Alloha дополнительно гео-блокирует по IP сервера, а не только по
  ...
  попадают под тот же блок — это не чинится правкой кода, только сменой
  исходящего IP запроса (residential/RU-прокси в `puppeteer.launch({args:
  ['--proxy-server=...']})`, если решат, что оно того стоит). По этой же
  причине AniLibria в этом проекте намеренно стримится в БРАУЗЕРЕ
  пользователя (§6), а не с сервера — единственный способ сохранить гео
  реального зрителя без прокси-инфраструктуры. Статус на момент записи:
  не проверено с реального Vercel IP (только вручную с IP не-РФ
  дата-центра) — возможно, конкретно `iad1` не под блоком, нужна проверка
  в проде.
