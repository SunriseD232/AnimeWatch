...
- **«Гео-блок» Alloha — ложный след: это анти-хотлинк проверка на `iframe`,
  а не блокировка по IP.** Страница Alloha содержит скрипт, который
  проверяет `window !== window.top` (открыта ли она именно ВНУТРИ
  `<iframe>`, как в реальном использовании на yani.tv) — если нет, скрипт
  СТИРАЕТ `<body>` и подставляет текст «Контент не найден или недоступен в
  вашем регионе». `page.goto(embedUrl)` в Puppeteer открывает URL как
  top-level документ — это как раз тот случай, поэтому даже с верным
  Referer выше страница выглядела гео-заблокированной. Никакой реальной
  проверки IP/страны нет — подтверждено: тот же URL с обычного (не-RU) IP,
  но открытый ВНУТРИ iframe, отдаёт настоящий плеер и реальный поток
  (`master.m3u8` с `*.vkvideo.cloud`). RU-прокси (см. ниже) тут был не
  нужен вообще — реальный фикс не имеет отношения к геолокации.

  Обход в `interceptVideoUrl` (`alloha.ts`): вместо `page.goto(embedUrl)`
  переходим на фиктивный `WRAPPER_URL` (реальный домен `yani.tv/...`,
  выбран для правдоподобного Referer) — сам запрос никогда не уходит в
...
- **RU-прокси для Puppeteer (`ALLOHA_PROXY_*`) остался в коде, но для
  Alloha не требуется** — см. пункт выше, проблема была не в гео. Логика
  полностью опциональна (без переменных окружения ничего не меняется) и
  может пригодиться для другого источника, если у него окажется настоящая
  IP-блокировка. HTTP(S)-порт присланного пользователем прокси на практике
  не отвечал (curl подключался, но зависал без ответа) — рабочим оказался
  только SOCKS5-порт; Chromium же не поддерживает авторизованные SOCKS5
  через `--proxy-server` (нет парсинга логина/пароля для этого протокола) —
  если прокси когда-нибудь понадобится по-настоящему, тянуть SOCKS5 в
  Chromium нужно будет через локальный мост (например, пакет
  `proxy-chain`), не напрямую.
