...
src/
  app/                        — роуты (App Router)
    page.tsx                  — главная (аниме): Продолжить + 3 плитки (§14)
    ...
    code/page.tsx               — код регистрации (§13), виден только конкретным email
    ...
    api/signup/, api/login/    — регистрация/вход через сервер (§13.2)
  ...
  components/                 — все React-компоненты (клиентские и серверные)
    GenreFilterPanel.tsx        — чипы жанров (3 состояния) + сортировка, общий для аниме/кино (§14)
  ...
  lib/
    ...
    concurrency.ts               — mapWithConcurrency, для догрузки карточек каталога (§14)
    signupCode.ts, rateLimit.ts, telegram.ts, tmdb.ts — см. §13/§7.1
...
| `TMDB_API_KEY` | опционально | рейтинг «Популярного» кино и трейлеры (§14) |
| `SIGNUP_CODE_SECRET` | да, иначе регистрация закрыта | код приглашения при регистрации (§13) |
...
регистрацию (fail closed, а не открытая для всех).
...
/catalog?genres=&exclude=&sort=&page=       — каталог аниме: жанры (AND/exclude) + сортировка (§14)
/anime/[shikimoriId]                       — страница тайтла аниме
/watch/[shikimoriId]/[episode]              — просмотр аниме
...
/cinema/catalog?genres=&exclude=&sort=&page= — каталог кино: жанры (AND/exclude) + сортировка (§14)
...
/code                                       — код регистрации (§13), виден ТОЛЬКО 2000gva@gmail.com
```

## 13. Доступ к сайту — регистрация по коду, полный auth-gate
...
### 13.2. Регистрация и вход — только через свои Route Handlers
...
`/api/signup` и `/api/login`: **5 запросов/мин**. Тот же best-effort
компромисс serverless Vercel, что и у `/api/download` в другой ветке —
живёт в рамках одного тёплого инстанса функции, не строгая гарантия между
инстансами/после cold start, но первая линия защиты от перебора. `AuthForm`
показывает оставшиеся попытки (`remaining` в ответе роута) прямо в форме.

## 14. Навигация и каталог (аниме + кино)
...
### 14.1. Множественный выбор, исключение, AND — ограничения API
...
### 14.3. «Популярное» кино — TMDB на весь пул
...
### 14.4. `GenreFilterPanel` — общий компонент
