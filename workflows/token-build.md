...
| **Custom script** | Small systems: a Node/Python transform (see `scripts/design/validate_tokens.py` for the parse model). |
...
- On change to `tokens/*.json`: run `scripts/design/validate_tokens.py` (valid JSON + aliases resolve), regenerate all platform artifacts, fail the build if generated output is stale (committed artifacts must match a fresh build).
- Run `scripts/design/contrast.py` to gate color changes against WCAG before publishing.
- Version artifacts with the system (`workflows/governance.md`).

## Verification
- `scripts/design/validate_tokens.py` passes (no unresolved aliases).
