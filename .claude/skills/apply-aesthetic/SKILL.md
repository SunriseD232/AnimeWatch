...
2. Pick a direction in `taste/aesthetic-systems.md`:
   ...
   - A **named library system** — browse with `python3 scripts/design/design_systems.py list` (or `search <term>` / `show <name>`); specs live in `design-systems/library/<name>/DESIGN.md`.
...
4. **Verify contrast** of every mapped color pair (`scripts/design/contrast.py` / `a11y-audit`). A brand value that fails must be adjusted — taste never overrides POUR.
