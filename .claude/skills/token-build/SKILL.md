...
2. Pick the tool: **Style Dictionary** (multi-platform, the default), **Tokens Studio** (Figma-owned tokens — pairs with `figma-integration`), W3C DTCG export, or a small custom script (model on `scripts/design/validate_tokens.py`).
...
5. Wire CI: on `tokens/*.json` change, run `scripts/design/validate_tokens.py`, regenerate, fail if committed artifacts are stale; gate colors with `scripts/design/contrast.py`.

## Verification (definition of done)
- `python3 scripts/design/validate_tokens.py` passes (no unresolved aliases).
