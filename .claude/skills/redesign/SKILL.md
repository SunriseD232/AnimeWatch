...
6. **Verify**: re-run `design-review` + `a11y-audit`; smoke-test every previously working flow; dark mode + responsive spot-check. Run `scripts/design/validate_contrast.py` on the token source and `scripts/design/lint_hardcodes.py` over the changed code.
