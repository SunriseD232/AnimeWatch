...
6. Optional fast start: `python3 scripts/design/scaffold_component.py "<Name>"` to emit a stub, then fill it in.
...
- `node scripts/design/verify_states.mjs <harness> [--dark]` — contrast of every element in default/hover/focus
- `node scripts/design/axe_audit.mjs <harness> [--dark]` — ARIA/role/name/label correctness
- `node scripts/design/measure_render.mjs <harness> [--dark]` — every text element AA
- overlays/modals also: `node scripts/design/verify_focustrap.mjs <harness> --open=<trigger>`
...
Build mobile-first; a fixed-px width that can't shrink is a bug. Run `node scripts/design/verify_responsive.mjs <file|dir>` — it loads each harness at 280/320/414px and fails on any horizontal overflow. The four recurring causes and their fixes:
