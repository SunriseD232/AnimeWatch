...
2. **Anchor to a known system** if it's close — browse `taste/aesthetic-systems.md` / `python3 scripts/design/design_systems.py search <term>` and adopt that recipe to stabilize decisions.
3. **Build the token theme** from the inferred values → 3-tier DTCG (`design-tokens` skill); generate a single `theme.css`. Verify every color pair with `scripts/design/contrast.py` / `scripts/design/validate_contrast.py` (light + dark) — a sampled brand color that fails AA gets adjusted; taste never overrides POUR.
...
5. **Verify against the reference** — render and screenshot it, compare side-by-side to the reference; run `node scripts/design/measure_render.mjs`, `lint_hardcodes.py`, `taste_audit.mjs`, and `npm run verify`.
