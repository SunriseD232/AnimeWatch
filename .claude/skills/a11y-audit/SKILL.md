...
4. **Contrast — measure, don't eyeball.** For rendered HTML, RUN the real-render gates and report their actual output (CLAUDE.md → Verification Protocol): `node scripts/design/measure_render.mjs <file> [--dark]` (every text element) AND `node scripts/design/verify_states.mjs <file> [--dark]` (every interactive element in default/hover/focus — catches hover-state failures). For loose color pairs, `python3 scripts/design/contrast.py "<fg>" "<bg>"`. Never state a ratio you did not measure.
