...
выполняется там же через `psql`. Строка подключения — `SUPABASE_DB_URL` в
серверном `.env`. Сервисный ключ через PostgREST **DDL не выполняет** —
через него миграцию не применить.
