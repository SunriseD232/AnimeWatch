...
final class ExternalDisplayManager: NSObject {
    ...
    private func detachWindow() {
        ...
        // РАНЬШЕ тут был stop(), который полностью убивал player. Судя по
        // поведению на устройстве (звук пропадает почти сразу после
        // блокировки, после разблокировки видео стоит на паузе на той же
        // позиции) — didDisconnectNotification, похоже, приходит не только
        // при физическом отключении очков, но и когда iOS временно отзывает
        // доступ к внешнему экрану у свёрнутого/заблокированного
        // приложения. Останавливать player тут — как раз то, что рвёт
        // воспроизведение при блокировке; ТЕПЕРЬ player продолжает играть в
        // фоне (окно/слой для показа временно нет, но сам плеер жив), и
        // attachWindow ниже переподключит именно этот player к новому окну,
        // если экран "переподключится" (в т.ч. просто на разблокировке).
        // Настоящая остановка — только по явной команде из JS, см.
        // ExternalDisplayPlugin.stop().
    }
