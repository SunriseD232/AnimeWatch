# MediaWatch — как работать в этом репозитории

Ниже, начиная с раздела «UX/UI Expert Agent», лежит установленный набор
plugin87/ux-ui-agent-skills (v2.5.1, MIT) — справочник по дизайн-системам,
доступности и компонентам. Он подключён 10.09.2026 как СПРАВОЧНИК И
ЧЕК-ЛИСТ, а не как устав проекта. Что это значит:

- **Правила проекта старше.** Язык кода и комментариев здесь русский,
  комментарий объясняет ПОЧЕМУ так сделано (см. любой файл в `src/`).
  Архитектура и эксплуатация описаны в `ARCHITECTURE.md` и `README.md` —
  они и есть источник правды по стеку, деплою и кронам.
- **Стек уже выбран**: Next.js 14 (App Router), React 18, Tailwind 3.4,
  self-hosted Supabase. Раздел набора про «любой фреймворк» — фон, а не
  повод что-то мигрировать.
- **DTCG-токенов в проекте нет.** Тема живёт в CSS-переменных
  (`src/app/globals.css`, `tailwind.config.ts`). Гейты набора, которые
  требуют `design-tokens.json` (`accuracy_report`, `validate_tokens`,
  `validate_contrast`), на этом репозитории не запускаются — им нечего
  проверять. Полезное из набора — то, что работает по живой странице:
  `verify_responsive`, `verify_overflow`, `verify_target_size`,
  `axe_audit`, `taste_audit`, `slop_tells`.
- **Скрипты набора лежат в `scripts/design/`**, а не в `scripts/`: там уже
  живёт `deploy.sh`. Пути внутри документов набора поправлены под это.
  Скриптам нужен playwright (`npm i -D playwright`), в зависимостях проекта
  его нет — ставить по необходимости, локально.
- **Запрет на эмодзи из набора относится к интерфейсу**, а не к переписке
  и не к существующим текстам. Переписывать уже написанное ради него не
  надо.
- Проверять правки — на живом проде (https://media-watch.ru) или локальным
  `npm run dev`, как и раньше. «Отрендерь и посмотри» из набора — верное
  требование, оно тут и так соблюдается.
...
2. **Verify ALL states, not just resting.** A button that looks fine at rest can fail on hover/focus/active (CSS specificity traps). For any rendered HTML, run `node scripts/design/verify_states.mjs <file> [--dark]` (real computed contrast in default/hover/focus) — not only `measure_render.mjs`.
3. **Run the one-command gate before reporting done:** `node scripts/design/accuracy_report.mjs` (= tokens + contrast + spec + no-hardcode + theme-refs + no-emoji + real-render WCAG + state-aware, light & dark). Report the actual `N/N` line. It is all-or-nothing.
...
| Request | Skill | Load |
|---------|-------|------|
| Generate/extend/validate tokens, palettes, theming | `design-tokens` | `tokens/*.json`, "Token System"; `scripts/design/validate_tokens.py` |
| Design or build a screen / component | `design-component` | `components/*`, `accessibility/aria-patterns.md`, `tokens/*`; `scripts/design/scaffold_component.py` |
...
| Accessibility / WCAG / contrast check | `a11y-audit` | `accessibility/*`; `scripts/design/contrast.py` |
| Apply a look / vibe / brand feel | `apply-aesthetic` | `taste/aesthetic-systems.md`, `design-systems/library/*`; `scripts/design/design_systems.py` |
| Reference image / screenshot / mockup → matching code | `image-to-code` | `taste/*`, `design-tokens` + `design-code`; `scripts/design/measure_render.mjs`, `scripts/design/taste_audit.mjs` |
| Build a brand design system / foundation from scratch | `brandkit` | "Token System" + "Color Generation", `taste/aesthetic-systems.md`; `scripts/design/validate_contrast.py` |
...
| Versioning / contribution / deprecation / add-or-promote a component | `governance` | `workflows/governance.md`; `scripts/design/validate_tokens.py` |
| Token build pipeline → CSS/Tailwind/iOS/Android (Style Dictionary, DTCG) | `token-build` | `workflows/token-build.md`; `scripts/design/validate_tokens.py` |
| Figma ↔ code sync, Variables, Code Connect, Figma MCP | `figma-integration` | `workflows/figma-integration.md` |
| QA gates / CI / visual regression / prevent regressions | `design-qa` | `workflows/design-qa.md`; `scripts/design/validate_contrast.py`, `scripts/design/lint_hardcodes.py` |
...
| Critique / taste verdict / "is this actually good" | `/critique` | `.claude/agents/design-critic.md`, `taste/*`; `scripts/design/taste_audit.mjs`, `scripts/design/slop_tells.mjs` |
...
Delete is a bug. Measured by `scripts/design/lint_intent.mjs`.
...
signal, never a score) by `scripts/design/taste_audit.mjs`.
...
templates/product-design/ ← Starter layout for a NEW product repo (CLAUDE.md brief, .mcp.json,
                            ...
                            or /scaffold-project. Gated by scripts/design/validate_template.py.
