...
export interface ResolvedStream {
  ...
  /**
   * Как переключать качество из `qualities`.
   *
   * По умолчанию (поле отсутствует) приложение собирает из них
   * многоуровневый master.m3u8 и отдаёт переключение hls.js — так работают
   * Videoseed и прочие, у которых варианты совместимы по границам сегментов.
   *
   * 'reload' — варианты НЕЛЬЗЯ смешивать в один манифест: каждый живёт по
   * своей подписанной ссылке, и CDN не принимает сегмент от другого варианта
   * (Alloha). Такое качество выбирается полной заменой источника через
   * ?q=<высота>, как отдельные .mpd у Aksor.
   */
