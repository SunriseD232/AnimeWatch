 ...
 *
 * Порядок вариантов — ОТ МЕНЬШЕГО К БОЛЬШЕМУ (сортируем сами, не доверяя
 * порядку qualities от вызывающего кода — например, Videoseed's
 * probeQualities отдаёт по убыванию). Разбор жалобы «на телефоне в Safari
 * загрузка секунд 20» (наш плеер, Videoseed): нативный HLS-движок Safari — в
 * отличие от hls.js с его собственной оценкой пропускной способности — на
 * холодном старте склонен пробовать ПЕРВЫЙ вариант в списке, и часто именно
 * его же ждёт для reference. Если первым стоит самое высокое качество (как
 * было раньше), это означает потенциально существенно более крупный сегмент,
 * которого нужно накопить достаточно на старте — особенно заметно на
 * мобильной сети. hls.js/desktop эту особенность не имеет, поэтому раньше
 * никто и не замечал — только Safari-специфичный симптом.
 */
