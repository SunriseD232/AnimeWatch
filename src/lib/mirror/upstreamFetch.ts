...
export async function fetchUpstream(
  ...
  const res = await wreqFetch(url, {
    ...
    // ВРЕМЕННО (диагностика): подтверждённо на Vercel node-wreq возвращает
    // status:200 с телом НАШЕГО собственного /login вместо апстрима, хотя
    // тот же вызов локально отдаёт настоящий контент — подозрение на DNS-
    // резолвинг внутри их Rust-рантайма в serverless-песочнице. Форсируем
    // публичные DNS явно, вместо системного резолвера по умолчанию.
