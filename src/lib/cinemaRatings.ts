...
/**
 * Потолок запросов за один прогон. 40 000 при ~12 rps — это примерно час.
 * Полная база (103 тысячи) закрывается за три недельных прогона, дальше
 * обновляются только те, у кого отметка протухла.
 */
const DEFAULT_BUDGET = 40_000;

/** Одновременных запросов к TMDB. Официальный лимит там ~50 rps, но всё
 *  идёт через туннель, и упираться в него незачем. */
