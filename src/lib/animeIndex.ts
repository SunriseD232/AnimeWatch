...
async function fetchPage(page: number): Promise<GqlAnime[]> {
  // censored: false — «не прятать 18+», обязательный флаг.
  //
  // Без него Shikimori МОЛЧА выкидывает такие тайтлы из постраничной выдачи:
  // ответ всё равно приходит полным (limit штук), просто цензурированные в
  // него не попадают, и заметить потерю по одному ответу невозможно.
  // Проверено вживую на четвёртой странице: без флага 0 тайтлов с рейтингом
  // rx, с флагом — 8 из тех же пятидесяти.
  //
  // Из-за этого после переезда каталога на локальный индекс из него исчезло
  // всё 18+ разом, а фильтры «Хентай», «Юри», «Яой», «Сёдзё-ай»,
  // «Сёнен-ай» и рейтинг RX отдавали пустоту: жанры в таксономии есть, а
  // тайтлов с ними в индексе не было ни одного.
  const data = await gql<{ animes: GqlAnime[] }>(
    `{ animes(limit: ${PAGE_SIZE}, page: ${page}, order: id, censored: false) { ${ANIME_FIELDS} } }`,
  );
  return data.animes ?? [];
}
