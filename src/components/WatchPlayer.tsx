...
export default function WatchPlayer({
  ...
  // --- Подбор источника: AniLibria → иначе Kodik ---
  useEffect(() => {
    let cancelled = false;
    setResolving(true);
  ...
  }, [animeRomaji, animeRussian, animeYear, episode]);
  ...
  }, [isAuthed, hasNext, shikimoriId, animeTitle, posterUrl, episode]);
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {showOtherBanner && otherEpisode !== null && (
        <div className="flex items-center justify-between gap-3 rounded-lg border border-accent/20 bg-accent/10 px-4 py-3 text-sm">
          ...
          <div className="flex items-center gap-2">
            <Link
              href={`/watch/${shikimoriId}/${otherEpisode}`}
      ...
      <div className="flex items-center gap-2">
        <Link
          href={hasPrev ? `/watch/${shikimoriId}/${episode - 1}` : '#'}
        ...
        <Link
          href={hasNext ? `/watch/${shikimoriId}/${episode + 1}` : '#'}
      ...
      {ended && hasNext && (
        <div className="flex items-center justify-between gap-3 rounded-lg border border-white/10 bg-bg-card px-4 py-3 text-sm">
          ...
          <Link
            href={`/watch/${shikimoriId}/${episode + 1}`}
