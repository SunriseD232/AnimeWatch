      ...
      if (isHls) {
        ...
        if (Hls.isSupported()) {
          ...
          const hls = new Hls({
            ...
            // startLevel:0 — воспроизведено вживую (та же жалоба, «Мятеж»
            // kp=5582050, резюм с 16с): даже убрав дублирующий ручной сик
            // (см. usesStartPosition выше), сам сик у hls.js на резюме
            // зависал намертво — video.seeking оставался true бесконечно,
            // буфер не продвигался дальше первых секунд с начала, хотя
            // currentTime уже стоял на 16. По сети было видно: hls.js
            // параллельно тянет ПЕРВЫЕ сегменты СРАЗУ НЕСКОЛЬКИХ уровней
            // качества (обычное автопробование пропускной способности при
            // startLevel:-1/auto) — и это пробование, похоже, конфликтует
            // именно с одновременным стартом НЕ с нуля (startPosition).
            // startLevel:0 — конкретный уровень с самого начала (не auto),
            // без этого пробования; 0 — гарантированно САМОЕ низкое
            // качество благодаря сортировке по возрастанию в
            // synthesizeMasterPlaylist (см. proxy.ts) — тот же список,
            // что hls.js получает и без него, просто не даём ему самому
            // перебирать варианты в момент резюма. Только на резюме — не
            // трогаем обычный старт с нуля, где такой проблемы не было.
            startLevel: usesStartPosition ? 0 : -1,
          });
          hlsRef.current = hls;
