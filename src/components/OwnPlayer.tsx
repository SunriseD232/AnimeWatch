        ...
        if (Hls.isSupported()) {
          ...
          hls.on(Hls.Events.MANIFEST_PARSED, (_evt, data) => {
            ...
            if (effectiveSource === 'alloha') {
              ...
              // Разбор жалобы «ресюм-сик виснет на Videoseed» (в т.ч. на
              // Safari) — воспроизведено вживую на «Мятеж» (kp=5582050)
              // ПОСЛЕ двух предыдущих попыток (не помогли): даже с
              // startLevel:0 hls.js по сети продолжал одновременно тянуть
              // сегменты СРАЗУ НЕСКОЛЬКИХ уровней качества, а буфер
              // застревал ровно на границе первого сегмента — тот же
              // паттерн, что hls.js Events.ERROR/bufferAppendError. У
              // Videoseed синтезированный master (см. synthesizeMasterPlaylist
              // в proxy.ts) собран из НЕЗАВИСИМО раздаваемых по URL
              // качеств — не гарантированно совместимых по границам
              // сегментов/меток времени между собой при переключении, как
              // требует настоящий ABR. Тот же приём, что у Alloha выше, но
              // мягче: фиксируем автоматический (ABR) переключатель на
              // САМОМ НИЗКОМ уровне сразу (последний в levels — массив
              // отсортирован по убыванию) — hls.js больше не переключает
              // уровень сам. Ручной селектор качества (в отличие от Alloha)
              // оставляем — сама смена качества уже идёт тем же способом
              // (hls.currentLevel = конкретный index, см. обработчик
              // селектора), там это не автоматическое ABR-переключение
              // посреди буферизации, а явное действие пользователя.
              if (levels.length > 0) hls.currentLevel = levels[levels.length - 1].index;
