        ...
        if (Hls.isSupported()) {
          ...
          hls.on(Hls.Events.MANIFEST_PARSED, (_evt, data) => {
            ...
            if (effectiveSource === 'alloha') {
              ...
              // Videoseed раньше (до 2026-09-03) жёстко фиксировался тут на
              // САМОМ НИЗКОМ уровне и без ABR вовсе — попытка №3 из серии
              // фиксов зависания ресюм-сика на «Мятеж» (kp=5582050), которая
              // сама по себе зависание НЕ решила (решило удаление startPosition
              // для Videoseed, см. applyResumeSeek/startPosition ниже) и с тех
              // пор просто заставляла каждого пользователя вручную поднимать
              // качество на каждой серии. Настоящая причина зависания уже
              // устранена отдельно — возвращаем обычный авто-ABR hls.js, как у
              // всех источников, кроме Alloha (там жёсткая фиксация остаётся
              // по СВОЕЙ отдельной причине — см. комментарий выше: подписанные
              // сегменты только под изначально выбранный вариант).
