 ...
 */
/**
 * ВАЖНО про выравнивание: у карусели НЕ должно быть ни отрицательных полей,
 * ни внутренних отступов.
 *
 * Раньше здесь было `-mx-4 ... px-4` — приём «во всю ширину экрана»: полоса
 * карточек выходила за поля страницы, а padding возвращал первую карточку на
 * линию заголовка. Он тянул за собой три беды сразу:
 *  - браузер приклеивал первую карточку к началу прокручиваемой области, а
 *    она padding не учитывает, и карусель молча оказывалась прокрученной
 *    ровно на величину отступа (scrollLeft = 16 при padding-left = 16);
 *  - полоса прокрутки рисовалась по всей ширине элемента, то есть заметно
 *    левее заголовка;
 *  - прокрученные карточки не обрезались краем элемента, а проезжали под
 *    padding — слева торчала полоска предыдущей карточки в несколько
 *    пикселей.
 *
 * Без отступов всё три исчезают разом: элемент ровно по ширине контента,
 * его край и обрезает, и держит полосу прокрутки.
 */
