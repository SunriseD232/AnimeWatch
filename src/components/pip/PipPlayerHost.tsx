    ...
    <PipHostContext.Provider value={contextValue}>
      ...
      <div
        ...
        style={{
          ...
          pointerEvents: 'none',
          zIndex: 30,
          // Фильтр-пустышка. Визуально не делает НИЧЕГО — opacity(1) — но
          // отрезает этот причал от общей подложки страницы: элемент с
          // filter становится собственным backdrop-root, и его содержимое
          // больше не участвует в том, что видят панели с backdrop-filter.
          //
          // Зачем. Здесь за экраном постоянно висит полноразмерное
          // НЕПРОЗРАЧНОЕ проигрывающееся видео (непрозрачность обязательна,
          // иначе браузер закроет настоящий PiP — см. выше). Видео, которое
          // играет, уезжает в аппаратный слой, а панели поверх страницы
          // (уведомления, поиск, дропдауны) в этот момент перестают получать
          // подложку и рисуются просто полупрозрачными вместо матового
          // стекла. Отсюда и «иногда»: ровно тогда, когда жив мини-плеер.
          //
          // Побочный эффект тут полезный: отфильтрованное поддерево
          // растеризуется в текстуру, то есть аппаратный оверлей для этого
          // видео не запрашивается вовсе. На 480x270 за экраном это ничего
          // не стоит, а настоящему PiP фильтр не мешает — он показывает
          // поток мимо композитора страницы.
          filter: 'opacity(1)',
        }}
      >
        {session &&
