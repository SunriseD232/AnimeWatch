...
# tsconfig.json правит не человек, а сам `next build`: он дописывает в
# "include" КОНКРЕТНЫЙ distDir этой сборки («.build-<метка>/types/**/*.ts»).
# Строки копятся от сборки к сборке, а каталоги старых сборок мы удаляем — и
# следующая сборка падает на «Cannot find module ... page.js», пытаясь
# прочитать типы из несуществующего каталога. Именно это ломало деплой три
# прогона подряд. Поэтому файл возвращаем в состояние репозитория ДО
# проверки на грязь: его правка — след сборки, а не чья-то работа.
git checkout -- tsconfig.json 2>/dev/null || true

# Всё остальное, что правили прямо на сервере, — тупик: git pull на таких
# правках падает на середине, а до этого сервер тихо работает не на том коде,
# что в репозитории. Отваливаемся сразу и говорим, что именно изменено.
# Учитываются только отслеживаемые файлы: ecosystem.config.js и posters/
# лежат рядом намеренно и в git не входят.
...
# См. выше: сборка только что дописала свой distDir в tsconfig.json. Убираем
# сразу, чтобы следующий прогон не спотыкался о собственный след.
git checkout -- tsconfig.json 2>/dev/null || true
